...
func (stateMachine *StateMachine) setupGrub(rootfsVolName string, rootfsPartNum int, efiPartNum int, efiBIOSHybrid bool, architecture string) (err error) {
	...
	addGrubInstallCmds(&setupGrubCmds, mountDir, target, efiBIOSHybrid, architecture, loopUsed)
...
func addGrubInstallCmds(cmds *[]*exec.Cmd, mountDir string, target string, efiBIOSHybrid bool, architecture string, loopUsed string) {
	*cmds = append(*cmds,
		execCommand("chroot",
			mountDir,
			"grub-install",
	...
	if architecture == arch.AMD64 && efiBIOSHybrid {
		*cmds = append(*cmds,
			execCommand("chroot",
				mountDir,
				"grub-install",
