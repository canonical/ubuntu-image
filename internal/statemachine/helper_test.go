...
func Test_eatmydataEnv(t *testing.T) {
	type args struct {
		env []string
	}
	tests := []struct {
		name              string
		args              args
		wantLdLibraryPath string
		wantLdPreload     string
	}{
		{
			name: "non empty vars",
			args: args{
				env: []string{
					"LD_LIBRARY_PATH=test1:test2",
					"LD_PRELOAD=test3",
				},
			},
			wantLdLibraryPath: "LD_LIBRARY_PATH=test1:test2:/usr/lib/libeatmydata",
			wantLdPreload:     "LD_PRELOAD=test3 libeatmydata.so",
		},
		{
			name: "absent vars",
			args: args{
			...
			wantLdLibraryPath: "LD_LIBRARY_PATH=/usr/lib/libeatmydata",
			wantLdPreload:     "LD_PRELOAD=libeatmydata.so",
		},
		{
			name: "empty vars",
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			asserter := helper.Asserter{T: t}
			gotLdLibraryPath, gotLdPreload := eatmydataEnv(tt.args.env)
			asserter.AssertEqual(tt.wantLdLibraryPath, gotLdLibraryPath)
			asserter.AssertEqual(tt.wantLdPreload, gotLdPreload)
		})
	}
}
