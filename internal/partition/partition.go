...
func GeneratePartitionTable(volume *gadget.Volume, sectorSize uint64, imgSize uint64, isSeeded bool) (partition.Table, int, int, bool, error) {
	...
	for i := range volume.Structure {
		...
		if err != nil {
			return nil, rootfsPartitionNumber, bootPartitionNumber, hasBIOSPartition, err
		}

		partitionNumber++
	}

	return partitionTable.GetConcreteTable(), rootfsPartitionNumber, bootPartitionNumber, hasBIOSPartition, nil
}
